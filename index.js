...
client.on('message', async message => {
    if(message.channel.id == "761074628189356034"){



        try {
            const msg = await message.channel.messages.fetch('795161078539354142');
            let str = msg.content;
            let arr = str.split(' ');
            let res = arr[4];
            let count = res.match(/\d*$/);
            await msg.edit("legend has it that " + res.substr(0, count.index) + (++count[0]) + " messages were lost in the void...");
        } catch(err) {
            console.error(err);
        }
        message.delete();
